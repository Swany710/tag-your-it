...
export default function NewRepPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(data.error ?? "Failed to assign rep slot.");
    setSaving(false);
  }
    ...
    <AdminShell>
      <div className="p-8 max-w-2xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-white">Assign Rep Slot</h1>
        </div>

        <div className="card mb-5">
          <p className="text-slate-300 text-sm">
            The system supports 25 fixed rep slots. Assign one slot to each rep, then manage it from the reps list.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="label">Rep Slot ID (1-25) *</label>
              <input
                type="number"
                className="input"
                value={form.id}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="5+ years helping homeowners get fair insurance claims..."
            />
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="submit" className="btn-primary" disabled={saving}>
              {saving ? "Assigning..." : "Assign Rep Slot"}
            </button>
